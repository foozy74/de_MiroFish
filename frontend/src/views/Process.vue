      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              ...
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} Kanten</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">Warte auf Ontologie-Generierung</p>
            <p class="waiting-hint">Nach Abschluss wird der Graphaufbau automatisch gestartet</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">Daten werden in Kürze angezeigt...</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Nach dem Hochladen analysiert das LLM die Dokumente und generiert automatisch eine geeignete Ontologiestruktur für die Meinungssimulation (Entitätstypen + Beziehungstypen)
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">Warte auf Ontologie-Generierung...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Schnittstellenbeschreibung</div>
                <div class="detail-content">
                  Basierend auf der generierten Ontologie werden Dokumente segmentiert, die Zep-API aufgerufen und ein Wissensgraph mit Entitäten und Beziehungen erstellt
                </div>
              </div>
              
              <!-- 等待本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Warte auf Abschluss der Ontologie-Generierung...</div>
              </div>
...
const goToNextStep = () => {
  // TODO: 进入环境搭建步骤
  alert('Umgebungsaufbau-Funktion wird entwickelt...')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Keine Dateien zum Hochladen gefunden – bitte zur Startseite zurückkehren'
    loading.value = false
    return
  }
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'Verarbeitungsfehler'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Graphaufbau konnte nicht gestartet werden'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Graphaufbau konnte nicht gestartet werden: ' + (err.message || 'Unbekannter Fehler')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || 'Verarbeitung läuft...'
      }
