      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: 'Graph', split: 'Zwei Spalten', workbench: 'Werkbank' }[mode] }}
          </button>
        </div>
      </div>
...
const handleGoBack = async () => {
  ...
  addLog('Rückkehr zu Schritt 2 – Simulation wird beendet...')
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Simulationsumgebung wird beendet...')
      try {
        await closeSimulationEnv({
          simulation_id: currentSimulationId.value,
          timeout: 10
        })
        addLog('✓ Simulationsumgebung beendet')
      } catch (closeErr) {
        addLog(`Simulationsumgebung konnte nicht beendet werden – zwangsweises Stoppen wird versucht...`)
        try {
          await stopSimulation({ simulation_id: currentSimulationId.value })
          addLog('✓ Simulation zwangsweise gestoppt')
        } catch (stopErr) {
          addLog(`Zwangsweises Stoppen fehlgeschlagen: ${stopErr.message}`)
        }
      }
    } else {
      // Umgebung läuft nicht – prüfen ob Prozess gestoppt werden muss
      if (isSimulating.value) {
        ...
        try {
          ...
          addLog(`Simulation konnte nicht gestoppt werden: ${err.message}`)
        }
      }
    }
  } catch (err) {
...
const handleNextStep = () => {
  ...
  // 这个方法仅作为备用
  addLog('Eintritt in Schritt 4: Bericht generieren')
}

// --- Data Logic ---
const loadSimulationData = async () => {
  try {
    addLog(`Simulationsdaten laden: ${currentSimulationId.value}`)
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Zeitkonfiguration nicht abrufbar – Standardwert wird verwendet: ${minutesPerRound.value} Min/Runde`)
      }
...
const startGraphRefresh = () => {
  ...
  addLog('Echtzeit-Graphaktualisierung gestartet (30 s)')
  // 立即刷新一次，然后每30秒刷新
...
const stopGraphRefresh = () => {
  if (graphRefreshTimer) {
    ...
    addLog('Echtzeit-Graphaktualisierung gestoppt')
  }
}
